...
model SteamTurbine_L2_BP2
  ...
  TRANSFORM.Fluid.FittingsAndResistances.MultiPort multiPort(redeclare package
    ...
    annotation (Placement(transformation(extent={{-86,48},{-78,68}})));
...
equation
  ...
  connect(actuatorBus.opening_TCV, TCV.opening) annotation (Line(
      points={{30.1,100.1},{30.1,74},{-64,74},{-64,24}},
      color={111,216,99},
  ...
  connect(BPT.shaft_b, generator.shaft)
    annotation (Line(points={{-2,46},{54,46},{54,40}},  color={0,0,0}));
  ...
  connect(sensorBus.W_total, sensorW.W) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(TCV.port_b, CT.portHP) annotation (Line(points={{-54,16},{-38,16},{
          -38,16},{-22,16}}, color={0,127,255}));
  connect(TCV_BP.port_b, BPT.portHP)
    annotation (Line(points={{-40,52},{-22,52}}, color={0,127,255}));
  connect(CT.portLP, condenser.port_a) annotation (Line(points={{-2,16},{52,16},
          {52,-20},{84,-20},{84,-29},{77,-29}}, color={0,127,255}));
  connect(BPT.portLP, prt_b_steamdump) annotation (Line(points={{-2,52},{4,52},
          {4,-100},{60,-100}}, color={0,127,255}));
  connect(CT.shaft_b, BPT.shaft_a) annotation (Line(points={{-2,10},{2,10},{2,
          32},{-26,32},{-26,46},{-22,46}}, color={0,0,0}));
  connect(actuatorBus.ProcessCV, TCV_BP.opening) annotation (Line(
      points={{30,100},{30,70},{-50,70},{-50,60}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(multiPort.port_a, port_a_steam)
    annotation (Line(points={{-86,58},{-86,60},{-100,60}}, color={0,127,255}));
  connect(multiPort.ports_b[1], TCV_BP.port_a) annotation (Line(points={{-78,
          56.6667},{-68,56.6667},{-68,52},{-60,52}}, color={0,127,255}));
  connect(multiPort.ports_b[2], TCV.port_a) annotation (Line(points={{-78,58},{
          -68,58},{-68,32},{-78,32},{-78,16},{-74,16}}, color={0,127,255}));
  connect(actuatorBus.Feed_Pump_Speed, FWCP.in_m_flow) annotation (Line(
      ...
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(steam_p.port, TCV_BP.port_a) annotation (Line(points={{-210,50},{-210,
          44},{-116,44},{-116,76},{-66,76},{-66,52},{-60,52}}, color={0,127,255}));
  connect(steam_T.port, TCV_BP.port_a) annotation (Line(points={{-200,86},{-200,
          76},{-66,76},{-66,52},{-60,52}}, color={0,127,255}));
  ...
  connect(sensorBus.Feedwater_Temp, FinalFeedwater_T.T) annotation (Line(
      points={{-30,100},{-30,-44},{-168,-44},{-168,-72},{-178,-72}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(multiPort.ports_b[3], nonLinear_Break.port_a) annotation (Line(points={{-78,
          59.3333},{-68,59.3333},{-68,32},{-78,32},{-78,0},{-76,0},{-76,-3}},
        color={0,127,255}));
  connect(nonLinear_Break.port_b, nTU_HX_SinglePhase.Shell_in) annotation (Line(
        points={{-76,-23},{-76,-67.2},{-66,-67.2}}, color={0,127,255}));
  connect(FHV.port_b, OFWH_2.port_a) annotation (Line(points={{-1.77636e-15,
          -122},{6,-122},{6,-74},{32,-74},{32,-60},{22,-60}}, color={0,127,255}));
  connect(nTU_HX_SinglePhase.Shell_out, nonLinear_Break1.port_a) annotation (
      Line(points={{-46,-67.2},{-44,-67.2},{-44,-95}}, color={0,127,255}));
  connect(nonLinear_Break1.port_b, FHV.port_a) annotation (Line(points={{-44,
          -115},{-44,-122},{-20,-122}}, color={0,127,255}));
  connect(ofwh_p.y, pump1.inputSignal) annotation (Line(points={{24.6,-30},{54,
          -30},{54,-52.7}}, color={0,0,127}));
