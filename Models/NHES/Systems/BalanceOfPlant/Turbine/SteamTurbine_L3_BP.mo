...
model SteamTurbine_L3_BP
  ...
  Fluid.Machines.Pump_Pressure                  pump1(
    ...
    use_input=true,
    p_nominal=data.P_ms - 10e5,
    eta=data.eta_p)
    annotation (Placement(transformation(extent={{64,-70},{44,-50}})));
  TRANSFORM.Fluid.Volumes.SimpleVolume OFWH(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    p_start=data.P_ms - 10e5,
    T_start=OFWH.Medium.saturationTemperature_sat(OFWH.Medium.setSat_p(data.P_cond)),
  ...
  TRANSFORM.Fluid.Valves.ValveLinear FHV(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    dp_nominal=500000,
    m_flow_nominal=data.mdot_fh)     annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={-10,-122})));
  ...
  MicroGrids.FWH   uA3HX(redeclare package Medium =
        Modelica.Media.Water.StandardWater,
    P_h_nom=data.P_ms,
    P_f_nom=data.P_ms - 10e5,
    T_hi_nom=data.T_ms,
    T_fi_nom=data.Thx_feed,
    TTD=data.TTD,
    DCA=data.DCA,
    m_f_nom=data.mdot_ms)
...
equation
  ...
  connect(OFWH.port_a, pump1.port_b)
    annotation (Line(points={{28,-60},{44,-60}}, color={0,127,255}));
  ...
  connect(generator.port, sensorW.port_a) annotation (Line(points={{100,38},{
          100,30}},                       color={255,0,0}));
  connect(sensorW.port_b, port_e) annotation (Line(points={{100,10},{100,0}},
                                            color={255,0,0}));

  connect(sensorBus.W_total, sensorW.W) annotation (Line(
      points={{-29.9,100.1},{120,100.1},{120,20},{111,20}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(sensorBus.Feedwater_Temp, FinalFeedwater_T.T) annotation (Line(
      ...
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(FHV.port_b, OFWH.port_a) annotation (Line(points={{-1.77636e-15,-122},
          {6,-122},{6,-74},{32,-74},{32,-60},{28,-60}}, color={0,127,255}));
  ...
  connect(OFWH.port_b, uA3HX.port_a_fw)
    annotation (Line(points={{16,-60},{6,-60}},   color={0,127,255}));
  connect(uA3HX.port_b_hs, FHV.port_a) annotation (Line(points={{-4,-70},{-34,
          -70},{-34,-122},{-20,-122}},
                       color={0,127,255}));
  connect(FinalFeedwater_T.port, port_b_feed) annotation (Line(points={{-184,-82},
          {-184,-88},{-100,-88},{-100,-60}}, color={0,127,255}));
  connect(uA3HX.port_b_fw, pump2.port_a)
    annotation (Line(points={{-14,-60},{-40,-60}}, color={0,127,255}));
  connect(port_a_steam, uA3HX.port_a_hs) annotation (Line(points={{-100,60},{
          -100,-44},{-4,-44},{-4,-50}}, color={0,127,255}));
  ...
  connect(port_a_steam, steam_T.port) annotation (Line(points={{-100,60},{-100,
          44},{-226,44},{-226,80},{-200,80},{-200,86}}, color={0,127,255}));
