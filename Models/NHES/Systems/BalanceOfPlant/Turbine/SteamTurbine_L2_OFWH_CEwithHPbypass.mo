...
model SteamTurbine_L2_OFWH_CEwithHPbypass
  ...
  TRANSFORM.Fluid.Volumes.Separator moistureSeperator(
    ...
    eta_sep=0.99,
    nPorts_a=2,
    nPorts_b=1) annotation (Placement(transformation(extent={{-2,48},{18,68}})));
...
equation
  ...
  connect(OFWH_1.port_a, moistureSeperator.port_Liquid) annotation (Line(points={{30,-60},
          {38,-60},{38,38},{26,38},{26,36},{4,36},{4,54}},
                                                      color={0,127,255}));
  ...
  connect(sensorBus.I_pressure, sensor_p1.p) annotation (Line(
      points={{-30,100},{-30,76},{8,76},{8,50},{22,50}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(actuatorBus.Divert_Valve_Position, lpTBV.opening) annotation (Line(
      ...
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(threeWayValveGrad.port_b, prt_b_steamdump) annotation (Line(points={{62,-12},
          {62,-46},{70,-46},{70,-86},{60,-86},{60,-100}},         color={0,127,
          255}));
  connect(threeWayValveGrad.port_a2, resistance1.port_b) annotation (Line(
        points={{52,-2},{28,-2},{28,20},{21,20}}, color={0,127,255}));
  connect(resistance1.port_a, TCV.port_a) annotation (Line(points={{7,20},{-34,
          20},{-34,40},{-72,40},{-72,60}}, color={0,127,255}));
  connect(sensor_p1.port, threeWayValveGrad.port_a1) annotation (Line(points={{28,40},
          {28,8},{62,8}},                 color={0,127,255}));
  connect(actuatorBus.powerset, threeWayValveGrad.y) annotation (Line(
      points={{30,100},{30,36},{80,36},{80,-2},{70,-2}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
