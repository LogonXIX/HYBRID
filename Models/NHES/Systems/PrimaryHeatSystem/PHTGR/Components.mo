...
package Components
  ...
  model CoreStartUp
    ...
    import TRANSFORM.Fluid.Types.LumpedLocation;
    import Modelica.Fluid.Types.Dynamics;
    parameter Modelica.Units.SI.Power Q_start=15e6;
    parameter Real Fr=1.1 "Radial Power Peaking Factor";
    Subchannel Center(
      redeclare package Medium = Medium,
      r_coolant=geometry.R_Coolant,
    ...
    TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_b(redeclare package Medium =
          Medium)
      annotation (Placement(transformation(extent={{90,-10},{110,10}})));
    Fluid.Ultilities.FlowMultiplier flowMultiplierTin(redeclare package Medium =
    ...
    Fluid.Ultilities.FlowMultiplier flowMultiplierEin(redeclare package Medium =
                 Medium, capacityScaler=1/geometry.nChannels[2])
      annotation (Placement(transformation(extent={{-44,10},{-24,30}})));
    Fluid.Ultilities.FlowMultiplier flowMultiplierCein(redeclare package Medium =
                 Medium, capacityScaler=1/geometry.nChannels[1])
      annotation (Placement(transformation(extent={{-44,-30},{-24,-10}})));
    Fluid.Ultilities.FlowMultiplier flowMultiplierCoin(redeclare package Medium =
                 Medium, capacityScaler=1/geometry.nChannels[3])
      annotation (Placement(transformation(extent={{-44,-70},{-24,-50}})));
    Fluid.Ultilities.FlowMultiplier flowMultiplierCoout(redeclare package Medium =
                 Medium, capacityScaler=geometry.nChannels[3])
      annotation (Placement(transformation(extent={{26,-70},{46,-50}})));
    Fluid.Ultilities.FlowMultiplier flowMultiplierCeout(redeclare package Medium =
                 Medium, capacityScaler=geometry.nChannels[1])
      annotation (Placement(transformation(extent={{26,-30},{46,-10}})));
    Fluid.Ultilities.FlowMultiplier flowMultiplierEout(redeclare package Medium =
                 Medium, capacityScaler=geometry.nChannels[3])
      annotation (Placement(transformation(extent={{26,10},{46,30}})));
    TRANSFORM.Nuclear.ReactorKinetics.PointKinetics_L1_powerBasedlimited
                                                                  kinetics(
      Q_nominal=Q_nominal,
      specifyPower=specifyPower,
      redeclare record Data_DH = Data_DH,
  ...
  parameter Boolean toggle_ReactivityFP=true
    ...
    Modelica.Blocks.Math.Gain gain1(k=1)
      annotation (Placement(transformation(extent={{84,-126},{68,-110}})));
    parameter SI.Time Trans=1e3 "Duration of ramp (= 0.0 gives a Step)";
    parameter SI.Time startTime=1e3 "Output y = offset for time < startTime";
  equation
    connect(flowMultiplier1.port_b, port_b)
      annotation (Line(points={{90,0},{100,0}}, color={0,127,255}));
    connect(port_a, sensor_m_flow.port_a)
    ...
    connect(gain1.y, fuelModel.Tin_max)
      annotation (Line(points={{67.2,-118},{9,-118}}, color={0,0,127}));
    connect(Total_Power.y, ASMdivision.u1)
      annotation (Line(points={{-207,4},{-207,0},{-192,0}}, color={0,0,127}));
    connect(fuelModel.Power_in, Total_Power.y) annotation (Line(points={{-11,
            -112},{-198,-112},{-198,-20},{-200,-20},{-200,0},{-207,0},{-207,4}},
          color={0,0,127}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
                                                               Ellipse(
            extent={{-92,30},{-108,-30}},
