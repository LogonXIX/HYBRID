...
model extractionTurbine_test
  ...
  NHES.Fluid.Machines.ExtractionTurbine extractionTurbine(m_ext=0)
  ...
  NHES.Fluid.Valves.FlowCV flowCV(
    ...
    FlowRate_target=0,
    dp_nominal=10000)
    annotation (Placement(transformation(extent={{8,-102},{28,-82}})));
  Modelica.Blocks.Sources.RealExpression realExpression(y=y)
    annotation (Placement(transformation(extent={{154,40},{134,60}})));
  parameter Modelica.Blocks.Interfaces.RealOutput y=20e5 "Value of Real output";
equation
  ...
  connect(boundary2.ports[1], flowCV.port_b)
    annotation (Line(points={{42,-90},{36,-90},{36,-92},{28,-92}},
                                                 color={0,127,255}));
  connect(flowCV.port_a, extractionTurbine.port_b_extraction)
    annotation (Line(points={{8,-92},{0,-92},{0,-20}}, color={0,127,255}));
  connect(realExpression.y, boundary1.p_in) annotation (Line(points={{133,50},{
          110,50},{110,48},{102,48}}, color={0,0,127}));
