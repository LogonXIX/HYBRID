...
model ExtractionTurbineModular
  ...
  parameter Real eta_t=0.9 "Turbine Isentropic Effiency" annotation (Dialog(group="Nominal Turbine Conditions"));
  ...
  parameter Integer nExt(min=1,max=3)=3 "Number of Extractions, Max=3";
    ...
    final parameter Modelica.Units.SI.AbsolutePressure P_ext2mod(fixed=false)=P_ext2;
    final parameter Modelica.Units.SI.AbsolutePressure P_ext3mod(fixed=false)=P_ext3;
  ...
  Valves.PressureCV pressureCV2(
    ...
    PID_Ti=20,
    m_flow_nominal=m_in - m_ext1 - m_ext2,
    dp_nominal=1000) if nExt>1 annotation (Placement(transformation(
        extent={{6,6},{-6,-6}},
        rotation=180,
        origin={8,6})));
  ...
  TRANSFORM.Fluid.FittingsAndResistances.TeeJunctionVolume tee1(
    ...
    port_3(m_flow(start=-m_ext2)))  if nExt>1
    ...
    port_3(m_flow(start=-m_ext3)))  if nExt>2
...
initial equation
  ...
  h_ext2=((h_out-h_in)/(s_out-s_in))*s_ext2+h_in-((h_out-h_in)/(s_out-s_in))*s_in;
  h_ext2=medium.specificEnthalpy_ps(P_ext2,s_ext2);
  h_out=h_ext2-(h_ext2-medium.specificEnthalpy_ps(P_out,s_ext2))*eta_3;
  d_ext2=medium.density_ph(P_ext2,h_ext2);
      //Third
  h_ext3=h_ext2;
  d_ext3=d_ext2;
  ...
  h_ext1=medium.specificEnthalpy_ps(P_ext1,s_ext1);
  h_ext1=h_in-(h_in-medium.specificEnthalpy_ps(P_ext1,s_in))*eta_1;
  h_out=h_ext1-(h_ext1-medium.specificEnthalpy_ps(P_out,s_ext1))*eta_2;
  d_ext1=medium.density_ph(P_ext1,h_ext1);
  h_ext3=h_ext2;
  d_ext3=d_ext2;
  s_ext3=s_ext2;
