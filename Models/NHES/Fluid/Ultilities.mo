...
package Ultilities
  ...
  model zeroVolumeHeater
    ...
    input Modelica.Units.SI.Temperature T=573.15;
    Modelica.Units.SI.Power Q;
    TRANSFORM.HeatAndMassTransfer.Interfaces.HeatPort_Flow port_h
  ...
  equation
    ...
    port_a.h_outflow = inStream(port_b.h_outflow)+Q/port_a.m_flow;
    port_a.p = port_b.p;
    ...
    port_a.C_outflow = inStream(port_b.C_outflow);
    port_b.C_outflow = inStream(port_a.C_outflow);
    annotation (defaultComponentName="Heater",Icon(coordinateSystem(preserveAspectRatio=false), graphics={
          Text(
          ...
          Line(
            ...
            thickness=1),
          Line(
            points={{0,0},{10,6},{-10,16},{10,26},{-10,36},{10,48},{-2,54}},
            color={255,0,0},
            thickness=0.5,
            smooth=Smooth.Bezier)}),
                               Diagram(
          coordinateSystem(preserveAspectRatio=false)));
  end zeroVolumeHeater;
end Ultilities;
