...
<p>If parameter <b>limitAtInit</b> = <b>false</b>, the limits at the output of this controller block are removed from the initialization problem which leads to a much simpler equation system. After initialization has been performed, it is checked via an assert whether the output is in the defined limits. For backward compatibility reasons <b>limitAtInit</b> = <b>true</b>. In most cases it is best to use <b>limitAtInit</b> = <b>false</b>. </p>
  ...
  public
    ...
    Modelica.Blocks.Sources.Ramp ramp(
      ...
      offset=0,
      startTime=delayTime)
      annotation (Placement(transformation(extent={{-98,142},{-78,162}})));
    parameter SI.Time trans_time "Duration of ramp (= 0.0 gives a Step)";
    Modelica.Blocks.Math.Product product1
      annotation (Placement(transformation(extent={{-20,120},{0,140}})));
    Modelica.Blocks.Math.Add add_input
    ...
    Modelica.Blocks.Sources.Ramp ramp1(
      ...
      offset=1,
      startTime=delayTime)
      annotation (Placement(transformation(extent={{-104,184},{-84,204}})));
    Modelica.Blocks.Math.Product product2
      annotation (Placement(transformation(extent={{-20,176},{0,196}})));
  initial equation
    if initType==InitPID.InitialOutput then
       y = y_start;
    end if;
  equation
    ...
    connect(addSat.u1, limiter.y) annotation (Line(points={{86,-38},{86,-14},{
            93,-14},{93,0}}, color={0,0,127}));
    connect(gain_u_s.y, product1.u2) annotation (Line(points={{-83.4,0},{-80,0},{-80,
            116},{-30,116},{-30,124},{-22,124}}, color={0,0,127}));
    connect(product1.y, add_input.u2) annotation (Line(points={{1,130},{28,130},{28,
            176},{38,176}}, color={0,0,127}));
    connect(add_input.y, addP.u2) annotation (Line(points={{61,182},{64,182},{64,78},
            {-14,78},{-14,36},{-78,36},{-78,44},{-72,44}}, color={0,0,127}));
    connect(gain_u_m.y, product2.u2) annotation (Line(points={{4.44089e-16,-79.4},
            {-36,-79.4},{-36,180},{-22,180}}, color={0,0,127}));
    connect(ramp1.y, product1.u1) annotation (Line(points={{-83,194},{-32,194},{-32,
            136},{-22,136}}, color={0,0,127}));
    connect(ramp.y, product2.u1) annotation (Line(points={{-77,152},{-78,152},{-78,
            178},{-54,178},{-54,192},{-22,192}}, color={0,0,127}));
    connect(product2.y, add_input.u1) annotation (Line(points={{1,186},{4,186},{4,
            188},{38,188}}, color={0,0,127}));
    connect(add_offset.y, y) annotation (Line(points={{121,70},{126,70},{126,0},{110,
            0}}, color={0,0,127}));
    annotation (defaultComponentName="PID",
      Icon(coordinateSystem(
          preserveAspectRatio=true,
